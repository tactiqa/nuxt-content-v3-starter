...
// Fetch the single blog post based on the current dynamic route path
const { data: post } = await useAsyncData(route.path, () =>
  queryCollection('blog') // Query the 'blog' collection defined in content.config.ts
...
// Handle cases where the blog post is not found (e.g., 404 error)
if (!post.value) {
